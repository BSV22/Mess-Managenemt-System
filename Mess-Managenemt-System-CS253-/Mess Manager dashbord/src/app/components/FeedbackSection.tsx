  ...
  useEffect(() => {
    const fetchFeedback = async () => {
      try {
        ...
        const response = await fetch('http://localhost:3000/api/manager/feedback', {
          headers: {
        ...
        if (data.success) {
          ...
          const transformedFeedback = data.data.map((feedback: any) => ({
            id: feedback.id,
            studentRollNo: feedback.studentRollNo,
            studentName: feedback.student ? feedback.student.name : 'Unknown Student',
            date: feedback.createdAt ? new Date(feedback.createdAt).toLocaleDateString() : new Date().toLocaleDateString(),
            rating: feedback.rating,
            category: feedback.category.toLowerCase().replace(' ', '-'),
            ...
            <div className="flex items-start justify-between mb-3">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-sm text-gray-600">({feedback.studentRollNo})</span>
                </div>
