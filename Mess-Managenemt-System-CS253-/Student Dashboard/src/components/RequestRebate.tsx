...
export function RequestRebate() {
  ...
  const fetchRebates = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/student/rebates`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
      if (data.success) {
        setPreviousRequests(data.data.map((item: any) => ({
          id: String(item.requestId),
          startDate: item.startDate,
          endDate: item.endDate,
          reason: item.reason,
          status: String(item.status).toLowerCase() as 'pending' | 'approved' | 'rejected',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${API_BASE_URL}/api/student/rebate`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ startDate: start.toISOString(), endDate: end.toISOString(), reason })
      });
      const data = await res.json();
      if (data.success) {
