...
export function Dashboard() {
  ...
  const [billInfo, setBillInfo] = useState({ monthlyCharge: 0, extrasPurchased: 0, totalDue: 0 });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const currentBdmr = settingsData.data?.bdmrValue || 150;
